...
if run_button and genes:
    ...
    **API/Automation:** ❌ Web and FTP downloads; search interface only for most users.
    ...
    # CTD (Comparative Toxicogenomics Database) - All Categories Search
    st.subheader("5️⃣ CTD (Comparative Toxicogenomics Database)")
    st.markdown("""
    **Summary:**
    Integrated chemical, gene, disease, pathwy, GO and etc associations for environmental health and toxicogenomics.
    **API/Automation:** ❌ Web UI for most users; `basicQuery.go` for URL-based discovery.
    """)
    st.markdown("**Explore all CTD associations per gene:**")
    for g in genes:
        ctd_url = f"https://ctdbase.org/basicQuery.go?bqCat=all&bq={urllib.parse.quote(g)}"
        st.markdown(f"- [{g} (all categories)]({ctd_url}) in CTD")

    st.info("Click above to see all gene/chemical/disease/pathway/GO relationships annotated for your query in CTD.")
    # DeepPurpose
    st.subheader("6️⃣ DeepPurpose (DTI & Drug-Target Deep Learning)")
    st.markdown("""
    **Summary:**
    Deep learning library and pipelines for drug-target interaction prediction and compound screening.
    **API/Automation:** ✅ Python package and web GUI, no public REST API.
    """)
    st.markdown("[DeepPurpose GitHub](https://github.com/kexinhuang12345/DeepPurpose)")

    # Hetionet
    st.subheader("7️⃣ Hetionet (Integrative Biomedical Knowledge Network)")
    st.markdown("""
    **Summary:**
    Heterogeneous network joining genes, diseases, drugs, pathways, and compounds with 2+ million relationships.
    **API/Automation:** ❌ Download and query via Neo4j or explore web.
    """)
    hetio_url = "https://het.io/network/"
    st.markdown(f"[🌐 Explore Hetionet Network]({hetio_url})")
    st.markdown("[Hetionet GitHub](https://github.com/hetio/hetionet)") 
...
default_scores = {
    ...
    "CDT": [5, 3, 4, 5, 5, 4, 1],
    "DeepPurpose": [2, 1, 2, 5, 5, 1, 1],
    "Hetionet": [5, 4, 5, 5, 5, 1, 2]
}
